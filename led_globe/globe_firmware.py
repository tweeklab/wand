...
def main(sm, ip):
    ...
    sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP,  inet_aton(MCAST_GRP) + inet_aton(ip))
...
def main2(sm, ip):
    while True:
        ...
        for led in range(NUM_LEDS):
            if random.choice([True, False, False, False, False, False, False]) and not prev_choice:
                prev_choice = True
...
def main4(sm, ip):
    ...
    while True:
        ...
        for current_bright in range(MIN_VAL, MAX_VAL, STEP):
            ...
            down_color = HSV2RGB(ACCENT_HUE, SAT, (MAX_VAL-(current_bright-MIN_VAL))/denominator)
